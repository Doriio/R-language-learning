...
# 碎石图确定因子数量
fa.parallel(item_data, fa = "pc", n.iter = 100, show.legend = TRUE, main = "Scree Plot")
# 进行探索性因素分析
...
# 3剔除不符合要求的选项
# 将因子载荷矩阵转换为数据框
loadings_df <- as.data.frame(efa_result$loadings)
loadings_df$item <- rownames(efa_result$loadings)  # 添加条目编号

# 筛选因子载荷低于0.40的条目
low_loadings <- loadings_df %>% 
  filter(abs(RC1) < 0.40 & abs(RC2) < 0.40 & abs(RC3) < 0.40)

# 筛选多重负荷条目（多个因子载荷 > 0.40，差异小于0.2）
multi_loadings <- loadings_df %>%
  filter((abs(RC1) > 0.40 & abs(RC2) > 0.40 & abs(RC1 - RC2) < 0.2) |
         (abs(RC1) > 0.40 & abs(RC3) > 0.40 & abs(RC1 - RC3) < 0.2) |
         (abs(RC2) > 0.40 & abs(RC3) > 0.40 & abs(RC2 - RC3) < 0.2))

# 合并所有问题条目
remove_items <- unique(c(low_loadings$item, multi_loadings$item))
