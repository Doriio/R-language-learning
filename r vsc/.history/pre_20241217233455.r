...
# 处理 Weight 列，替换 "保密" 为 NA，删除 "kg" 并转换为数值
  mutate(
    Weight = ifelse(Weight == "保密", NA, Weight),   # 将 "保密" 替换为 NA
    Weight = gsub("kg", "", Weight),                  # 删除 "kg" 单位
...
# 删除体重保密的被试
  ...
  filter(!is.na(Weight))  # 删除 Weight 列为 NA 的行（即原来是 "保密" 或其他无效值的行）
