...
#####项目分析#####
# 难度分析  计算每个题项的平均得分
difficulty_scores <- colMeans(data[, paste0("Q", 1:20)], na.rm = TRUE) / 最高分  # 替换最高分为实际的最高分值
print(difficulty_scores)

# 区分度分析 计算每个题项的区分度
discrimination_scores <- sapply(data[, paste0("Q", 1:20)], function(x) {
    total_minus_item <- rowSums(data[, paste0("Q", 1:20)], na.rm = TRUE) - x
    cor(x, total_minus_item, use = "complete.obs")
})
print(discrimination_scores)

# 探索性因素分析
efa_result <- psych::fa(data[, paste0("Q", 1:20)], nfactors = 你估计的因子数, fm = "minres", rotate = "varimax")
print(efa_result)