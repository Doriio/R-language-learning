...
model_dif <- mirt::multipleGroup(
  ...
  parprior = list(
    list(par = "MEAN", group = "1", value = NA),  # 释放组1的均值
    list(par = "VAR", group = "1", value = NA),   # 释放组1的方差
    list(par = "MEAN", group = "2", value = NA),  # 释放组2的均值
    list(par = "VAR", group = "2", value = NA)    # 释放组2的方差
  )
)
...
# 进行 DTF 分析
dtf_results <- mirt::DTF(model_dif, groups = data_dif$gender)
# 查看 DTF 结果
print(dtf_results)