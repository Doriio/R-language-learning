...
# 确保列名正确，并排除非响应列（如 'Attention'，如果不需要处理）
# 如果 'Attention' 是响应列，则将其加入到相应的计分类别
# positive_items <- c(positive_items, "Attention")  # 根据需要决定是否加入

# 步骤 1：将响应列转换为字符类型并去除前后空格
data_mapped <- data_cleaned %>%
  mutate(across(c(positive_items, reverse_items), ~ str_trim(as.character(.))))

# 步骤 2：将响应文本映射为数值
data_mapped <- data_mapped %>%
  mutate(across(c(positive_items, reverse_items), ~ case_when(
    . == "完全不同意" ~ -3,
    . == "大部分不同意" ~ -2,
    . == "有些不同意" ~ -1,
    . == "有些同意" ~ 1,
    . == "大部分同意" ~ 2,
    . == "完全同意" ~ 3,
    . %in% c("无法选择", "拒绝回答", "不知道", "不适用") ~ NA_real_,  # 特殊值设为 NA
    TRUE ~ NA_real_  # 其他未定义的响应也设为 NA
  )))

# 步骤 3：对反向计分的列进行反向赋分
data_scored <- data_mapped %>%
  mutate(across(reverse_items, ~ ifelse(!is.na(.), -., .)))

# 步骤 4：将特殊值和 NA 替换为列的均值
data_filled <- data_scored %>%
  mutate(across(c(positive_items, reverse_items), ~ {
    # 计算当前列的均值，忽略 NA
