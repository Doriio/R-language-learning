...
for (i in item_columns) {
  t_test <- t.test(highgroup[[i]], lowgroup[[i]])
  t_results$CR[t_results$Item == i] <- abs(t_test$statistic)  # 取t统计量的绝对值
  t_results$p_value[t_results$Item == i] <- t_test$p.value
}
# 查看检验结果
print(t_results)
# 剔除出p值 > 0.05 的条目
significant_items <- t_results %>% filter(p_value < 0.05)
...
# 计算每个条目与量表总分的相关系数
for (i in item_columns) {
  cor_test <- cor.test(sample1[[i]], sample1$ATOP_total, method = "pearson")  # Pearson相关
