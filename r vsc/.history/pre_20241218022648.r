...
data_clean <- na.omit(data[, c("1":"20")])

data_clean <- data[, c("1":"20")]
for (i in 1:ncol(data_clean)) {
    data_clean[is.na(data_clean[, i]), i] <- mean(data_clean[, i], na.rm = TRUE)
}

sd_zero_cols <- sapply(data_clean, sd, na.rm = TRUE) == 0
data_clean <- data_clean[, !sd_zero_cols]
# 绘制屏幕图
fa.parallel(data[, c("1":"20")], fa = "fa", n.iter = 100, show.legend = TRUE, main = "Scree Plot")
