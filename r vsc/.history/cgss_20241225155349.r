...
# 转换 gender, region, education 为因子
data_cfa$gender <- as.factor(data_cfa$gender)
data_cfa$region <- as.factor(data_cfa$region)
...
# 删除education为NA的行
data_cfa_clean <- data_cfa %>%
  filter(!is.na(education))
# 检查处理后的数据
summary(data_cfa_clean$education)
data_cfa_clean$education <- as.factor(data_cfa_clean$education)
# 配置恒等性
configural_fit_education <- cfa(model, data=data_cfa_clean, group="education")
summary(configural_fit_education, fit.measures=TRUE)
# 度量恒等性
metric_fit_education <- update(configural_fit_education, group.equal="loadings")
summary(metric_fit_education, fit.measures=TRUE)
# 标量恒等性
scalar_fit_education <- update(metric_fit_education, group.equal=c("loadings", "intercepts"))
summary(scalar_fit_education, fit.measures=TRUE)
# 比较模型拟合度
anova(configural_fit_education, metric_fit_education, scalar_fit_education)

# 拟合模型
fit <- cfa(model, data=data_cfa)
summary(fit, fit.measures=TRUE)
# region分组的 MGCFA
fit_region <- cfa(model, data=data_cfa, group="region")
summary(fit_region, fit.measures=TRUE)
# 配置恒等性
configural_fit_region <- cfa(model, data=data_cfa, group="region")
# 度量恒等性
metric_fit_region <- update(configural_fit_region, group.equal="loadings")
# 标量恒等性
scalar_fit_region <- update(metric_fit_region, group.equal=c("loadings", "intercepts"))
# 比较模型拟合度
anova(configural_fit_region, metric_fit_region, scalar_fit_region)