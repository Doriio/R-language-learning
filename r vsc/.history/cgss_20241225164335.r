...
##### DIF DTF #####
data_dif_origin <- data_combined[, c("l2503", "l2505", "l2507", "l2509", "l2511", "l2513", "l2515", "gender")]
data_dif <- data_combined[, c("l2503", "l2505", "l2507", "l2509", "l2511", "l2513", "l2515", "gender")]
# 确保gender为因子类型
data_dif$gender <- as.factor(data_dif$gender)
model_dif <- mirt(data_dif, 1, itemtype = "graded")


# 定义grouping variable，假设性别为gender列，且已经是因子型数据
DIFresults <- mirt::DIF(model_dif, grouping = data_dif$gender)
# 查看DIF分析结果
print(DIFanalysis)
