...
# 提取项目参数
item_params <- coef(model_unconstrained, IRTpars = TRUE)

# 绘制每个条目的类别响应曲线（Category Response Curve, CRC）
for (i in 1:7) {
  plot(model_unconstrained, type = "trace", which.items = i, main = paste("Category Response Curve for Item", i))
}

# 使用iteminfo()绘制每个条目的项目信息曲线（Item Information Curve, IIC）
for (i in 1:7) {
  item_info <- iteminfo(model_unconstrained, items = i)  # 注意这里改用了 items 参数
  plot(item_info, type = "l", main = paste("Item Information Curve for Item", i))
}
