...
# 使用列名作为条目
items_to_plot <- c("l2503", "l2505", "l2507", "l2509", "l2511", "l2513", "l2515")

# 然后继续绘图
for (i in items_to_plot) {
  plot(model_unconstrained, type = "trace", which.items = i, main = paste("Category Response Curve for", i))
}
...
for (i in items_to_plot) {
  cat("Item", i, "\n")
  print(table(data_efa2[, i]))  # 查看该条目的类别分布
}
...
# 绘制指定条目的类别响应曲线，并加上标题
items_to_plot <- c(3, 5, 7, 9, 11, 13, 15)
# 绘制7个条目的类别响应曲线（Category Response Curves）
for (i in items_to_plot) {
  plot(model_unconstrained, type = "trace", which.items = i, main = paste("Item", i))
}
