...
# 检测难度 ("d") 和区分度 ("a") 的 DIF
DIFresults <- mirt::DIF(model_dif, which.par = "a1")
print(DIFresults)
DIFresults_d <- mirt::DIF(model_dif, which.par = c("d1", "d2", "d3", "d4"))
print(DIFresults_d)
DIFresults_all <- mirt::DIF(model_dif, which.par = c("a1", "d1", "d2", "d3", "d4"))
print(DIFresults_all)
# 绘制第一个条目的ICC（分组对比）
plot(model_dif, type = "trace", which.items = 1, facet_items = TRUE)
...
# 查看第 9 题的参数（l2509）
coef(model_dif)$items[4, ]

# 查看第 11 题的参数（l2511）
coef(model_dif)$items[5, ]

# 查看第 13 题的参数（l2513）
coef(model_dif)$items[6, ]