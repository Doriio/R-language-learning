...
model_dif <- mirt::multipleGroup(
  data_dif_origin,
  model = 1,
  itemtype = "graded",
  group = data_dif$gender,
  invariance = c("slopes", "intercepts"),  # 保持其他约束
  parprior = list(
    list(par = "MEAN", group = "1", value = NA),  # 释放组1均值
    list(par = "VAR", group = "1", value = NA),   # 释放组1方差
    list(par = "MEAN", group = "2", value = NA),  # 释放组2均值
    list(par = "VAR", group = "2", value = NA)    # 释放组2方差
  )
)
...
model_dif <- mirt::multipleGroup(
  ...
  itemtype = "graded",
  group = data_dif$gender,
  invariance = c("slopes", "intercepts"),  # 仅约束斜率和截距
  parprior = list(
    list(par = "MEAN", group = "1", value = NA),  # 释放组1均值
    list(par = "VAR", group = "1", value = NA),   # 释放组1方差
    list(par = "MEAN", group = "2", value = NA),  # 释放组2均值
