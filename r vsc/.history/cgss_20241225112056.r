...
# 拟合一个分级响应模型（Graded Response Model）
model <- mirt(data_efa2, model = 1, itemtype = 'graded')

# 绘制类别响应曲线 (CRCs)
for (i in 1:ncol(data_efa2)) {
    plot(model, type = 'trace', item = i)
}

# 绘制项目信息曲线 (IICs)
for (i in 1:ncol(data_efa2)) {
    plot(model, type = 'info', item = i)
}

# 打开一个新的图形窗口
# if (dev.cur() == 1) windows()  # 对于 Windows 用户
if (dev.cur() == 1) quartz()  # 对于 macOS 用户
# if (dev.cur() == 1) x11()  # 对于 Linux 用户

# 你的绘图代码
plot(model, type = 'trace', item = 1)  # 示例绘制第一个条目的 CRC
plot(model, type = 'info', item = 1)  # 示例绘制第一个条目的 IIC
plot(model, type = 'info', item = 7)  # 示例绘制第一个条目的 IIC

# 对每个条目绘制项目信息曲线
# 改正后的代码应该是
for (i in seq_len(ncol(data_efa2))) {
    plot(model, type = 'info', item = i, main = paste("Item Information Curve for", colnames(data_efa2)[i]))
}