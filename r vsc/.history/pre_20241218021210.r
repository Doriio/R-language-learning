...
# 从因子分析结果中提取因子载荷矩阵，并转换为数据框
loadings_matrix <- efa_result$loadings  # 提取载荷矩阵
loadings_df <- as.data.frame(loadings_matrix)  # 转换为数据框

# 添加题目编号（从行名获取）
loadings_df$item <- rownames(loadings_matrix)

# 查看列名，确保正常提取
print(colnames(loadings_df))
...
# 绘制因子载荷图
library(ggplot2)
ggplot(loadings_long, aes(x = item, y = loading, fill = loading > 0.3)) +
    geom_col() +
    facet_wrap(~factor) +
    theme_minimal() +
