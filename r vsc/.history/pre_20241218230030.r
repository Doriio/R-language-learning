...
# 提取样本1的条目数据
item_data <- sample1[, c("1", "4", "6", "7", "10", "14", "15", "16", "18", "19", "20")]
# KMO 检验
...
# 3剔除不符合要求的选项
# 剔除条目4，进一步检查双重负荷条目
item_data_filtered <- item_data %>% select(-c("4"))
# 重新运行EFA，提取3个因子
...
# 提取剔除问题条目后的数据
item_data_clean <- sample2[, c("Q1", "Q6", "Q7", "Q10", "Q14", "Q15", "Q16", "Q18", "Q19", "Q20")]

# 2. 定义模型
cfa_model <- '
  Factor1 =~ Q10 + Q15 + Q16 + Q6 + Q20
  Factor2 =~ Q1 + Q7
'
# 3. 运行 CFA
...
data$Factor1total <- rowSums(data[, c("10", "15", "16", "6", "20")], na.rm = TRUE)  # 因子1的题目
data$Factor2total <- rowSums(data[, c("1", "7")], na.rm = TRUE)  # 因子2的题目
...
cor_results <- data.frame(
  Comparison = c("ATOPtotal vs DBStotal", "Factor1total vs DBStotal", "Factor2total vs DBStotal"),
  Correlation = c(cor_atop_dbs$estimate, cor_factor1_dbs$estimate, cor_factor2_dbs$estimate),
  p_value = c(cor_atop_dbs$p.value, cor_factor1_dbs$p.value, cor_factor2_dbs$p.value)
)
print(cor_results)
...
#####信度检验#####
# ATOP量表
total_items <- data[, c("1", "2", "3", "4", "5", "6", "7", "9", "10", 
...
# 提取区分度和难度参数
item_difficulty <- sapply(item_parameters[-length(item_parameters)], function(x) x$par[2:4])
item_discrimination <- sapply(item_parameters[-length(item_parameters)], function(x) x$par[1])

# 创建参数表格
parameter_table <- data.frame(
  Item = names(item_parameters[-length(item_parameters)]),
  Discrimination = item_discrimination,
  Difficulty_1 = item_difficulty[1, ],  # 阈值1
  Difficulty_2 = item_difficulty[2, ],  # 阈值2
  Difficulty_3 = item_difficulty[3, ]   # 阈值3
)
