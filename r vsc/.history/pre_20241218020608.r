...
# 可视化因子载荷
# 将因子载荷转换为数据框以便绘图
loadings_data <- as.data.frame(efa_result$loadings)

# 给数据框添加题项标签
loadings_data$item <- rownames(loadings_data)

# 将数据框从宽格式转换为长格式，以适应 ggplot2
loadings_long <- tidyr::pivot_longer(loadings_data, cols = -item, names_to = "factor", values_to = "loading")

# 确保 loadings_long 数据框正确加载
print(loadings_long)

# 绘制因子载荷图
ggplot(loadings_long, aes(x = factor, y = loading, fill = loading > 0.3)) +
  geom_col() +
  facet_wrap(~item) +
  theme_minimal() +
  labs(title = "Factor Loadings", x = "Factor", y = "Loading") +
  scale_fill_manual(values = c("grey", "blue"), guide = FALSE)  # 只显示显著的载荷
