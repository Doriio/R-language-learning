...
# 获取题目列表
items <- colnames(data_efa2)  # 假设 data_efa2 是您的数据集
...
for (item in items) {
  # 绘制 CRC（分类反应曲线）
  png(filename = paste0("plots/CRC_", item, ".png"), width = 800, height = 600)
  plot(model_unconstrained, type = "trace", item = item, main = paste("Category Response Curve -", item))
  dev.off()
  
  # 绘制 ICC（项目特征曲线）
  png(filename = paste0("plots/ICC_", item, ".png"), width = 800, height = 600)
  plot(model_unconstrained, type = "info", item = item, main = paste("Item Characteristic Curve -", item))
  dev.off()
}

plot(model_unconstrained, type = "trace", item = 1, main = "Test CRC for Item 1")
plot(model_unconstrained, type = "info", item = 1, main = "Test ICC for Item 1")
plot(model_unconstrained, type = "info", item = 2, main = "Test ICC for Item 2")

# 获取所有条目的名称
items <- colnames(data_efa2)
...
for (item in items) {
  # 保存为PNG文件
  png(filename = paste0("plots/IIC_", item, ".png"), width = 800, height = 600)
  
  # 绘制项目信息曲线
  plot(model_unconstrained, type = "info", item = item, main = paste("Item Information Curve -", item))
