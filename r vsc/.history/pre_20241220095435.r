...
# 剔除条目，进一步检查双重负荷条目
item_data_filtered <- item_data %>% select(-c("7", "12", "19"))
...
item_data <- data[, c("1", "5", "9", "10", "15", "16", "20")]
...
# 设置图形参数
par(mfrow = c(1, 1),  # 一个图形窗口显示一个图
    mar = c(5, 5, 3, 3),  # 调整边距
    cex.axis = 1.5,  # 坐标轴数字放大
    cex.lab = 1.5)  # 坐标轴标签放大
...
plot(gpcm_model, 
  ...
  # 添加自定义坐标轴
  axis(1, at = seq(-6, 6, 2), cex.axis = 1.5, lwd = 2)  # X 轴，放大刻度值
  axis(2, cex.axis = 1.5, lwd = 2)                     # Y 轴，放大刻度值
...
# 添加自定义坐标轴
axis(1, cex.axis = 1.5, lwd = 2)  # X 轴，刻度值放大
axis(2, cex.axis = 1.5, lwd = 2)  # Y 轴，刻度值放大
