...
# 处理 Weight 列，替换 "秘密" 为 NA，删除 "kg" 并转换为数值
  mutate(
    Time = as.numeric(gsub("秒", "", Time)),
    Weight = ifelse(Weight == "秘密", NA, Weight),   # 将 "保密" 替换为 NA
    Weight = gsub("[^0-9.]", "", Weight),  # 删除所有非数字和小数点的字符
    Weight = as.numeric(Weight), # 转换为数值型
    Age = gsub("[^0-9]", "", Age),  # 删除所有非数字字符
    Age = ifelse(Age == "", NA, Age),  # 如果是空字符串，将其替换为 NA
    Age = as.numeric(Age)  # 将清理后的 Age 列转换为数值型
    Age = as.numeric(Age), # 确保 Age 转换为数值型
    ID = as.numeric(ID),          # 转换 ID 为数值型
    Attention = as.numeric(Attention),        # 转换 Attention 为数值型
