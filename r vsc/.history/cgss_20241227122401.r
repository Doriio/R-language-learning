...
# 打印并行分析的结果
print(fa_parallel$pc.values)  # 查看主成分分析的特征根
# 计算多项式相关矩阵
...
# 假设每个条目的卡方值表示其对测验的偏差贡献
total_chi <- sum(dif_stats$chi12, na.rm = TRUE)
# 打印总测验偏差
print(total_chi)
# 绘制所有条目的 DIF 曲线
plot(lordif_results)
...
dif_stats <- lordif_results$stats
# 总 DTF（使用 McFadden R²）
total_dtf_mcfadden <- sum(dif_stats$pseudo12.McFadden, na.rm = TRUE)
# 总 DTF（使用 Nagelkerke R²）
total_dtf_nagelkerke <- sum(dif_stats$pseudo12.Nagelkerke, na.rm = TRUE)

# 打印结果
print(paste("Total DTF (McFadden):", total_dtf_mcfadden))
print(paste("Total DTF (Nagelkerke):", total_dtf_nagelkerke))

# 计算 McFadden 和 Nagelkerke R² 的差异
dif_stats$R2_diff <- abs(dif_stats$pseudo12.McFadden - dif_stats$pseudo12.Nagelkerke)

# 打印每个条目的差异
print(dif_stats[, c("item", "pseudo12.McFadden", "pseudo12.Nagelkerke", "R2_diff")])

# 比较 McFadden 和 Nagelkerke 的伪 R²
barplot(
  height = rbind(dif_stats$pseudo12.McFadden, dif_stats$pseudo12.Nagelkerke),
  beside = TRUE,
  names.arg = paste("Item", dif_stats$item),
  legend.text = c("McFadden", "Nagelkerke"),
  args.legend = list(x = "topright"),
  main = "Comparison of Pseudo R² (McFadden vs Nagelkerke)",
  xlab = "Items",
  ylab = "Pseudo R²",
...
# 提取题目数据和性别分组变量
data_dif <- data_combined[, c("l2503", "l2505", "l2507", "l2509", "l2511", "l2513", "l2515", "gender")]
data_dif$gender <- as.factor(data_dif$gender)  # 确保 gender 是因子

library(mirt)
# 创建多组模型并检测 DIF
model_dif <- multipleGroup(data_dif[, -8], 1, itemtype = "graded", group = data_dif$gender)
DIFresults_all <- DIF(model_dif, which.par = c("a1", "d1", "d2", "d3", "d4"))
print(DIFresults_all)

# 绘制 DIF 项目的项目特征曲线
plot_items <- c(4, 5, 6)  # l2509, l2511, l2513
for (item in plot_items) {
  plot(model_dif, type = "trace", which.items = item, facet_items = TRUE, main = paste("Trace Plot - Item", item))
}

# 提取模型参数并计算跨组差异
coef_model <- coef(model_dif, simplify = TRUE)
group1_params <- coef_model[[1]]$items
group2_params <- coef_model[[2]]$items

# 计算区分度 (a1) 和难度参数 (d1-d4) 的差异
dif_results <- data.frame(
  Item = rownames(group1_params),
  DIF_a1 = group2_params[, "a1"] - group1_params[, "a1"],
  DIF_d1 = group2_params[, "d1"] - group1_params[, "d1"],
...
# 使用 lordif 进行 DIF 和 DTF 分析
data_dif_origin <- as.matrix(data_dif[, -8])
gender_numeric <- as.numeric(data_dif$gender)
library(lordif)
lordif_results <- lordif(data_dif_origin, gender_numeric)
summary(lordif_results)

# 提取 DTF 信息
dtf_stats <- lordif_results$stats
dtf_summary <- data.frame(
  Item = dtf_stats$item,
  Chi2 = dtf_stats$chi12,
  McFadden_R2 = dtf_stats$pseudo12.McFadden,
  Nagelkerke_R2 = dtf_stats$pseudo12.Nagelkerke,
  R2_Diff = abs(dtf_stats$pseudo12.McFadden - dtf_stats$pseudo12.Nagelkerke)
)
print(dtf_summary)

# 总 DIF 和 DTF 指标
total_dif_mcfadden <- sum(dtf_stats$pseudo12.McFadden, na.rm = TRUE)
total_dif_nagelkerke <- sum(dtf_stats$pseudo12.Nagelkerke, na.rm = TRUE)
total_chi <- sum(dtf_stats$chi12, na.rm = TRUE)

cat("Total DIF (McFadden R²):", total_dif_mcfadden, "\n")
cat("Total DIF (Nagelkerke R²):", total_dif_nagelkerke, "\n")
cat("Total Chi²:", total_chi, "\n")

# 绘制 DIF 曲线和 R² 比较图
plot(lordif_results)
barplot(
  height = rbind(dtf_stats$pseudo12.McFadden, dtf_stats$pseudo12.Nagelkerke),
  beside = TRUE,
  names.arg = paste("Item", dtf_stats$item),
  legend.text = c("McFadden", "Nagelkerke"),
  args.legend = list(x = "topright"),
  main = "Comparison of Pseudo R² (McFadden vs Nagelkerke)",
  xlab = "Items",
