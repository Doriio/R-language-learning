...
for (i in 1:7) {
  # 提取当前条目的参数
  item_param <- item_params$items[[i]]
  # 使用 ggplot2 绘制类别响应曲线
  plot_data <- data.frame(
    Theta = seq(-4, 4, by = 0.1),  # 潜在特质的值
    Response = predict(model_unconstrained, newdata = data.frame(Theta = seq(-4, 4, by = 0.1)), type = "response")[, i]
  )
  ggplot(plot_data, aes(x = Theta, y = Response)) +
    geom_line() +
    labs(title = paste("Category Response Curve for Item", i), x = "Theta", y = "Probability") +
    theme_minimal()
}
