...
ggplot(iris, aes(x = Sepal.Length, y = Sepal.Width, color = Species)) +
  ...
  coord_flip() # horizontal bar chat
...
ggplot(barley, aes(x = year, y = variety, fill = yield)) +
  ...
  facet_wrap(facets = vars(site), ncol = 1) + # 经常用这个
  # Add a fill scale using an 2-color gradient
  scale_fill_gradient(low = "white", high = "red")

# facet_grid() 用于创建一个网格布局，面板按行和列排列。你可以指定行（rows）和列（cols）的变量，从而形成一个网格状的面板。
# facet_wrap()适用于单个分类变量,面板按指定的分类变量进行排列，并自动换行。你可以控制每行显示的面板数量。